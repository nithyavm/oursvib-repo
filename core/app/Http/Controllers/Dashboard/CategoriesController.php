...
//changed by nithya
       ...
       // $fatherSections = Section::where('webmaster_id', '=', $webmasterId)->orderby('row_no', 'asc')->get();
    ...
    public function store(Request $request, $webmasterId)
    {
        ...
        $BookingType = implode(', ', $request->booking_type);
        else
        ...
        $Section->booking_type= $BookingType;
        $Section->visits = 0;
        $Section->status = 1;
    ...
    public function edit($webmasterId, $id)
    {
        ...
        if (!empty($Sections)) {
            ...
            return view("dashboard.categories.edit",
                compact("Sections", "GeneralWebmasterSections", "WebmasterSection", "fatherSections", "BookingType"));
        } else {
    ...
    public function getBooking(Request $request)
    {
       // return $request->father_id;
         ...
         foreach ($BookingType as $booking) {
          ...
          $Bookings_type= Section::where('webmaster_id', '=','12')->whereIn('id', explode(',' ,$bookings))->get();

        if (count($Bookings_type) > 0) {
            return response()->json($Bookings_type);
        }
    }
    ...
    {
        $SubCategory = Section::where('webmaster_id', '=','10')->where('father_id', '=' , $request->father_id)->get();
    ...
    public function  getParent(Request $request) 
    {
       // return "test";
        ...
        $categories= Section::find($parent_id);
        
        if ($categories) {
            
            if ($categories->father_id == 0) {
                return $categories->title_en;
            } else {
                $request->parent_id = $categories->father_id;
                //return "test";

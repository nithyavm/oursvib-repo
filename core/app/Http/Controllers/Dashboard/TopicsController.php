...
//order, paginate
    ...
    public function create($webmasterId)
    {
        ...
        $GeneralWebmasterSections = WebmasterSection::where('status', '=', '1')->orderby('row_no', 'asc')->get();
        // General END
        $BookingType = Section::where('webmaster_id', '=','12')->where('father_id', '=' , '0')->get();
        //Webmaster Topic Details
        $WebmasterSection = WebmasterSection::find($webmasterId);
        if (!empty($WebmasterSection)) {
            ...
            return view("dashboard.topics.create",
                compact("GeneralWebmasterSections", "WebmasterSection", "fatherSections", "BookingType"));
        } else {
            return redirect()->route('NotFound');
        }
    }
    ...
    public function store(Request $request, $webmasterId)
    {
        ...
        if (!empty($WebmasterSection)) {
           ...
           // return $request;
            ...
            if (isset($request->strategic_location1)) {
            $Listing = new Listing;
            $Listing->list_id =  $Topic->id;
    ...
    public function edit($webmasterId, $id)
    {
        ...
        $bookandbill=BookAndBill::where('topic_id' , $id)->get();
        ...
        if (!empty($WebmasterSection)) {
            ...
            $Packages = Section::where('webmaster_id', '=','13')->where('father_id','=',0)->whereIn('created_by', ['1',Auth::user()->id])->get();    
            $Capacity = Topic::where('webmaster_id', '=','18')->where('father_id','=',0)->whereIn('created_by', ['1',Auth::user()->id])->get();
            $Ammenitites = Topic::where('webmaster_id', '=','16')->whereIn('created_by', ['1',Auth::user()->id])->get();
            $Facility = Topic::where('webmaster_id', '=','17')->whereIn('created_by', ['1',Auth::user()->id])->get();           
            } else{
            ...
            $Capacity = Section::where('webmaster_id', '=','18')->where('father_id','=',0)->get();
            $Ammenitites = Topic::where('webmaster_id', '=','16')->get();
            $Facility = Topic::where('webmaster_id', '=','17')->get();       
            }
            ...
            if (@Auth::user()->permissionsGroup->view_status) {
               ...
               // $Listing = Topic::where('created_by', '=', Auth::user()->id)->find($id);
            } else {
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($Topic)) {
                    ...
                    if($WebmasterSection->title_en == 'Listings')
                    {
                            ...
                            if ($request->tab_name === 'tab_additional') {                                
                                $Additional_fee=AdditionalFee::where('topic_id' , $id)->delete();
                            ...
                            if(isset($request->booking_type)){
                                ...
                                $booking_type_count = count($request->input('booking_type'));
                                    for($i=0;$i<$booking_type_count;$i++){                                    
                                        ...
                                        $addbookandbill->bill_type =$request->input('billing_type')[$i];
